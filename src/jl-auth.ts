 ...
 *
 * Reverse-engineered from libjl_auth.so (ARM64). Tables extracted from offsets
 * 0x1b4c (KS_TABLE), 0x1c4c (SBOX), 0x1d4c (ISBOX). Verified byte-exactly
 * against a captured BLE auth exchange:
 *   challenge 70 B7 59 92 E0 5E A7 8F EC 53 3B A1 29 79 B5 90
 *   response  FF E9 E6 C8 0C E1 F4 0F 5C CE AE 20 83 1C 58 79
 *
 * Ported from hybridherbst/web-bluetooth-e87 (MIT). The static key appears to be
 * universal for this SDK revision; if a future device customizes it, re-extract
 * from that device's libjl_auth.so at the same offset.
 */
...
/** Build a [0x00, rand*16] auth init packet (17 bytes). */
...
/** Low-level: compute the 16-byte response for a raw 16-byte challenge. */
export function encryptChallenge(challenge16: Uint8Array): Uint8Array {
  const out = functionE1test(MAGIC, Array.from(challenge16), STATIC_KEY)
